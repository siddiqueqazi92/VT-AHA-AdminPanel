...
class TransferPaymentButton extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  handleClick = () => {
    ...
    const url = API_URL + `/${RESOURCES.WITHDRAWAL_REQUESTS}/approve`;
    ...
    fetch(url, {
      ...
      .then((response) => {
        ...
        if (response.status == RESPONSE_STATUS.UPDATED) {
          showNotification("Payment is on its way");
        } else {
      ...
      .catch((e) => {
        console.error(e);
        showNotification("Error: Payment could not be transfered", "warning");
      });
  };
  ...
  render() {
    ...
    return (
      <>
        <Button
          label="ra.strings.transfer_payment"
        ...
        <Confirm
          ...
          title="ra.strings.transfer_payment"
...
TransferPaymentButton.propTypes = {
  push: PropTypes.func,
